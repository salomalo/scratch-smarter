...
class WC_Memberships_Admin {
	...
	public function get_screen_ids( $context = null ) {
		...
		$settings_page_id   = SV_WC_Plugin_Compatibility::normalize_wc_screen_id();
	...
	private function init_modals() {
		...
		if ( $screen = get_current_screen() ) {
			...
			} elseif ( isset( $_GET['tab'], $_GET['section'] ) && 'email' === $_GET['tab'] && in_array( $_GET['section'], array( 'wc_memberships_user_membership_ending_soon_email', 'wc_memberships_user_membership_renewal_reminder_email' ), true ) && SV_WC_Plugin_Compatibility::normalize_wc_screen_id() === $screen->id ) {
