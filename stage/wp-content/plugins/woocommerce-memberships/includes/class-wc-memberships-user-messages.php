...
class WC_Memberships_User_Messages {
	 ...
	 * @since 1.10.0
	 *
	 * @param \WP_Post|\WC_Product $content content type
	 * @return string HTML
	 */
	public static function get_admin_message_html( $content ) {
		 ...
		 * @since 1.10.1
		 *
		 * @param bool $display_message default true
		 */
		$display_message = (bool) apply_filters( 'wc_memberships_display_restricted_content_admin_notice', true );

		if ( $display_message && current_user_can( 'wc_memberships_access_all_restricted_content' ) ) {

			$post = null;

			if ( $content instanceof WP_Post ) {
				$post         = $content;
				$access_rules = wc_memberships()->get_rules_instance()->get_post_content_restriction_rules( $post->ID );
			} elseif( $content instanceof WC_Product ) {
				$post         = get_post( $content->get_id() );
				$access_rules = wc_memberships()->get_rules_instance()->get_product_restriction_rules( $content->get_id() );
			}

			if ( $post && ! empty( $access_rules ) ) {

				$post_type      = get_post_type_object( $post->post_type );
				$post_type_name = $post_type && isset( $post_type->labels, $post_type->labels->singular_name ) ? $post_type->labels->singular_name : '';
				$post_type_name = ucwords( '' !== trim( $post_type_name ) ? $post_type_name : __( 'Content', 'woocommerce-memberships' ) );
				$plans          = array();

				foreach ( $access_rules as $rule ) {

					if ( $plan = $rule->get_membership_plan() ) {

						$plans[ $plan->get_id() ] = '<a href="' . esc_url( get_edit_post_link( $plan->get_id() ) ) . '">' . esc_html( $plan->get_name() ) . '</a>';
					}
				}

				if ( ! empty( $plans ) ) {

					/* translators: Placeholders: %1$s - content name (e.g. Post, Product), %2$s - a list of plan names linking to their edit screens */
					$message = sprintf( _n( 'Hey there! This %1$s is visible to you as an administrator, but it will be restricted for guests and non-members. Only members of %2$s will be able to access it normally.', 'Hey there! This %1$s is visible to you as an administrator, but it will be restricted for guests and non-members. Only members of the following plans will be able to access it normally: %2$s.', count( $plans ), 'woocommerce-memberships' ), $post_type_name, wc_memberships_list_items( $plans ) );
				}
			}
		}

		return ! empty( $message ) ? '<div class="woocommerce woocommerce-info">' . $message . '</div>' : '';
	}
	...
	private static function get_restricted_content_excerpt( $post, $message_code = '' ) {
		...
		if ( in_array( get_post_type( $post ), array( 'product', 'product_variation' ), true ) ) {
			...
			$excerpt = self::trim_excerpt( $post );
		}
