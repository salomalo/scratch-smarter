...
class WC_Memberships_Modal_Grant_Access_Membership_Plan extends WC_Memberships_Batch_Job_Modal {
	...
	protected function get_progress_bar() {
		...
		<small>
			<# if ( data.job.total == 1 ) { #>
				...
				printf( __( 'Processed %s out of 1 user.', 'woocommerce-memberships' ), '<span class="job-progress-current">{{data.job.progress}}</span>', '<span class="job-progress-total">{{data.job.total}}</span>' ); ?>
			<# } else { #>
				<?php /* translators: Placeholders: %1$s - current progress (number), %2$s - total amount of users to process (number) */
				printf( __( 'Processed %1$s out of %2$s users.', 'woocommerce-memberships' ), '<span class="job-progress-current">{{data.job.progress}}</span>', '<span class="job-progress-total">{{data.job.total}}</span>' ); ?>
			<# } #>
		</small>
		<?php
	...
	protected function get_job_completed() {
		...
		$opening_link = '<a href="' . esc_url( admin_url( 'edit.php?s&post_type=wc_user_membership&post_parent=' ) ) . '{{data.plan.id}}">';
		$closing_link = '</a>';
		...
		<p>
			...
			<# if ( data.job.total > 1 ) { #>
				<?php /* translators: Placeholders: %1$s - users processed, %2$s - opening <a> HTML tag, %3$s closing </a> HTML tag */
				printf( __( 'Processed %1$s users. %2$sView memberships%3$s.', 'woocommerce-memberships' ), '{{data.job.total}}', $opening_link, $closing_link ); ?>
			<# } else { #>
			<?php /* translators: Placeholders: %1$s - opening <a> HTML tag, %2$s closing </a> HTML tag */
				printf( __( 'Processed 1 user. %1$sView memberships%2$s.', 'woocommerce-memberships' ), $opening_link, $closing_link ); ?>
			<# } #>
		</p>
		<?php
