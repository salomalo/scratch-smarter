...
return 0;}});$.fbuilder.controls['fslider']=function(){};$.extend($.fbuilder.controls['fslider'].prototype,$.fbuilder.controls['ffields'].prototype,{title:"Slider",ftype:"fslider",predefined:"",predefinedMin:"",predefinedMax:"",predefinedClick:false,size:"small",thousandSeparator:",",centSeparator:".",min:0,max:100,step:1,range:false,minCaption:"",maxCaption:"",caption:"{0}",_getAttr:function(attr)
{var me=this,f,v=$.trim(me[attr]);if($.isNumeric(v))return parseFloat(v);f=(/^fieldname\d+$/i.test(v))?me.getField(v):false;if(f)
{v=f.val();if($.isNumeric(v))return parseFloat(v);}
return 0;},_setHndl:function(attr,one)
{var me=this,v=$.trim(me[attr]);if($.isNumeric(v))return;var s=(/^fieldname\d+$/i.test(v))?'[id*="'+v+me.form_identifier+'"]':v,i=(one)?'one':'on';$(document)[i]('change',s,function(){if(me['set_'+attr])me['set_'+attr](me._getAttr(attr));});},_setThousandsSeparator:function(v)
...
{return'<div class="fields '+this.csslayout+' '+this.name+' cff-slider-field" id="field'+this.form_identifier+'-'+this.index+'"><label for="'+this.name+'">'+this.title+'</label><div class="dfield slider-container"><input id="'+this.name+'" name="'+this.name+'" class="field" type="hidden" value="'+$.fbuilder.htmlEncode(this._getAttr('predefined'))+'"/><div id="'+this.name+'_slider" class="slider '+this.size+'"></div><div class="corner-captions '+this.size+'"><span class="left-corner">'+this.minCaption+'</span><span class="right-corner" style="float:right;">'+this.maxCaption+'</span></div><div id="'+this.name+'_caption" class="slider-caption"></div><span class="uh">'+this.userhelp+'</span></div><div class="clearer"></div></div>';},set_min:function(v)
{var e=$('[id="'+this.name+'_slider"]'),c=this.val(),r=false;e.slider('option','min',v);if($.isArray(c)){if(c[0]<v){c[0]=v;r=true;}}
else if(c<v){c=v;r=true;}
if(r)this.setVal(c);},set_max:function(v)
{var e=$('[id="'+this.name+'_slider"]'),c=this.val(),r=false;e.slider('option','max',v);if($.isArray(c)){if(v<c[1]){c[1]=v;r=true;}}
else if(v<c){c=v;r=true;}
if(r)this.setVal(c);},set_step:function(v)
{$('[id="'+this.name+'_slider"]').slider("option","step",v);},after_show:function()
...
{var me=this,v=$.trim(me[attr]);if(/^fieldname\d+$/i.test(v))
{var s='[id*="'+v+me.form_identifier+'"]',i=(one)?'one':'on';$(document)[i]('change',s,function(){if(me['set_'+attr])me['set_'+attr](me._getAttr(attr));});}},_set_Events:function()
...
$('#'+me.name).val(str).change();},set_minDate:function(v)
{var e=$('[id*="'+this.name+'"].hasDatepicker');if(e.length)
{e.datepicker('option','minDate',v);e.change();}},set_maxDate:function(v)
{var e=$('[id*="'+this.name+'"].hasDatepicker');if(e.length)
{e.datepicker('option','maxDate',v);e.change();}},set_DefaultDate:function()
