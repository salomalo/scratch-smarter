...
class WC_Memberships_Products_Restrictions {
	...
	/** @var int[] memoization of IDs of restricted products browsed by admins */
	private $product_restricted_admin_notice = array();
				...
				switch ( $restrictions->get_restriction_mode() ) {
					...
					case 'redirect' :
						...
						if (    isset( $_GET['wc-ajax'] )
						     ...
						     && SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.2' ) ) {
	...
	public function password_protect_restricted_product() {
		...
		// if the current user is an admin, tell them they might be viewing a restricted product
		if ( $product && current_user_can( 'wc_memberships_access_all_restricted_content' ) && ! in_array( $product->get_id(), $this->product_restricted_admin_notice, false ) ) {

			echo WC_Memberships_User_Messages::get_admin_message_html( $product );

			$this->product_restricted_admin_notice[] = $product->get_id() ;
		...
		} elseif ( $post && wc_memberships_is_product_viewing_restricted( $post ) && ! post_password_required() ) {
