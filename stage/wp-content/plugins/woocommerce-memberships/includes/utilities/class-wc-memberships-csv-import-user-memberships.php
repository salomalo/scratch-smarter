...
class WC_Memberships_CSV_Import_User_Memberships extends WC_Memberships_Job_Handler {
	...
	public function create_job( $attrs ) {
		...
		$rows = -2; // accounts for first row of headers
	...
	public function process_job( $job, $items_per_batch = 3 ) {
		...
		if ( $job->progress >= $job->total || $job->position >= $job->file_size || ! is_numeric( $current_position ) ) {
			...
			$results->html = $this->get_job_results_html( $job );
	 ...
	 * @return string HTML
	 */
	private function get_job_results_html( $job ) {
		...
		if ( 0 === $total_rows ) {
			...
			if ( $processed_rows > 0 ) {
				...
				$message .= '<p><span class="dashicons dashicons-no"></span>' . __( 'However, no User Memberships were created or updated with the given options.', 'woocommerce-memberships' ) . '</p>';
			}
		}
