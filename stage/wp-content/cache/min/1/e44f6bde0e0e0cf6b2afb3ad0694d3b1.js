...
jQuery(function(){jQuery('#lakes').find('path').attr({'fill':us_config['default']['lakesfill']}).css({'stroke':us_config['default']['lakesoutline']});});function addEvent(id,relationId){var _obj=jQuery('#'+id);var _Textobj=jQuery('#'+id+','+'#'+us_config[id]['visnames']);var _h=jQuery('#map').height();jQuery('#'+['visnames']).attr({'fill':us_config['default']['visnames']});_obj.attr({'fill':us_config[id]['upclr'],'stroke':us_config['default']['borderclr']});_Textobj.attr({'cursor':'default'});if(us_config[id]['enbl']==true){if(isTouchEnabled()){_Textobj.on('touchstart',function(e){var touch=e.originalEvent.touches[0];var x=touch.pageX-10,y=touch.pageY+(-15);var maptipw=jQuery('#tipus').outerWidth(),maptiph=jQuery('#tipus').outerHeight(),x=(x+maptipw>jQuery(document).scrollLeft()+jQuery(window).width())?x-maptipw-(20*2):x
